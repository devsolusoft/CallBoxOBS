...
document.addEventListener("DOMContentLoaded", () => {
  ...
  window.addEventListener("storage", (e) => {
    try {
      // If currentBox is set, react to changes for that box; in any case, also react to intake batch changes
      if (!e.key) return;
      if ((currentBox && e.key === storageKey(currentBox.id)) || (typeof e.key === 'string' && e.key.startsWith('llamador:intake:batch:'))) {
    ...
    channel.onmessage = (ev) => {
      try {
        if (ev?.data?.type === "STATE_UPDATED") {
          // Si no hay box seleccionado, o el mensaje es global (boxId == null), o coincide con la box actual -> refrescar
          if (!currentBox || ev.data.boxId == null || ev.data.boxId === currentBox.id) {
            renderPatientTable();
          }
        }
        if (ev?.data?.type === 'NEW_PATIENT' && ev.data && ev.data.data) {
          // Opcional: si llega NEW_PATIENT desde otro origen, podriamos refrescar o encolar.
          // No forzamos render aquí porque el batch se guarda en localStorage y el evento storage maneja la actualización.
        }
      } catch(e){}
    };;
  }
